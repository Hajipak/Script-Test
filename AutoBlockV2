...
LoadingSubtitle = "by YAR/Zen and Skibidi399",
ConfigurationSaving = {
Enabled = true,
...
local autoBlockAudioDelay = 0.25
local nextAudioBlockTime = 0
...
AutoBlockTab:CreateInput({
    Name = "AutoBlockAudio Delay (seconds)",
    PlaceholderText = tostring(autoBlockAudioDelay),
    RemoveTextAfterFocusLost = false,
    Flag = "AutoBlockAudioDelayInput",
    Callback = function(Text)
        local num = tonumber(Text)
        if num then
            autoBlockAudioDelay = math.max(0, num)
            print("[AutoBlockAudio] Delay set to:", autoBlockAudioDelay)
        else
            warn("[AutoBlockAudio] Invalid delay value provided:", tostring(Text))
        end
    end,
})

AutoBlockTab:CreateKeybind({
    Name = "Controller Auto Block Toggle",
    CurrentKeybind = "ButtonX", -- Default tombol X pada controller
    HoldToInteract = false,
    Flag = "ControllerAutoBlockKeybind", -- Flag unik untuk konfigurasi
    Callback = function(Keybind)
        -- Fungsi yang dijalankan saat keybind ditekan
...
AutoBlockTab:CreateParagraph({
    Title = "Controller Keybind",
    Content = "Use the keybind above to toggle Auto Block Audio with your controller."
})

AutoBlockTab:CreateSection("AutoBlock Control Floating")

-- Floating Toggle Audio
AutoBlockTab:CreateToggle({
    Name = "Enable AutoBlockAudio + Floating Button",
    CurrentValue = false,
    Flag = "AutoBlockAudioFloating",
    Callback = function(Value)
        autoBlockAudioOn = Value
        if Value then
            floatingGuiAudio = Instance.new("ScreenGui")
            floatingGuiAudio.Name = "FloatingAutoBlockAudio"
            floatingGuiAudio.ResetOnSpawn = false
            floatingGuiAudio.ZIndexBehavior = Enum.ZIndexBehavior.Sibling
            floatingGuiAudio.Parent = game:GetService("CoreGui")
            ...
            Button.Text = "Audio: ON"
            Button.Parent = floatingGuiAudio
            ...
            Button.MouseButton1Click:Connect(function()
                autoBlockAudioOn = not autoBlockAudioOn
                Button.Text = autoBlockAudioOn and "Audio: ON" or "Audio: OFF"
                StarterGui:SetCore("SendNotification", {
                    Title = "AutoBlock Audio",
                    Text = autoBlockAudioOn and "Enabled" or "Disabled",
                    Duration = 2
                })
            end)
        else
            if floatingGuiAudio then
                floatingGuiAudio:Destroy()
                floatingGuiAudio = nil
            end
        end
    end,
})
...
AutoBlockTab:CreateToggle({
    Name = "Enable AutoBlockAnimation + Floating Button",
    CurrentValue = false,
    Flag = "AutoBlockAnimationFloating",
    Callback = function(Value)
        autoBlockOn = Value
        if Value then
            floatingGuiAnim = Instance.new("ScreenGui")
            floatingGuiAnim.Name = "FloatingAutoBlockAnimation"
            floatingGuiAnim.ResetOnSpawn = false
            floatingGuiAnim.ZIndexBehavior = Enum.ZIndexBehavior.Sibling
            floatingGuiAnim.Parent = game:GetService("CoreGui")
            ...
            Button.MouseButton1Click:Connect(function()
                ...
                Button.Text = autoBlockOn and "Animation: ON" or "Animation: OFF"
                StarterGui:SetCore("SendNotification", {
                    Title = "AutoBlock Animation",
                    Text = autoBlockOn and "Enabled" or "Disabled",
                    Duration = 2
                })
            end)
        else
            if floatingGuiAnim then
                floatingGuiAnim:Destroy()
                floatingGuiAnim = nil
            end
        end
    end,
...
AutoBlockTab:CreateSection("Change Type Block")
...
BDTab:CreateSlider({
    ...
    Range = {1, 30},       -- Diubah dari {1, 16} menjadi {1, 30}
    Increment = 1,
    Suffix = "parts",
    CurrentValue = 4,
    Flag = "AntiFlickParts",
...
BDTab:CreateInput({
    ...
    Callback = function(text)
        local num = tonumber(text)
        if num then
            antiFlickDelay = math.max(0, num) -- donÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€¦Ã‚Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€¦Ã‚Â¾ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¢t allow negative
        end
    end,
})

BDTab:CreateParagraph({
    Title = "Recommended Delay Values",
    Content = [[
Coolkidd: 0.1ms
recommend: 0ms
...
BDTab:CreateInput({
    ...
    Callback = function(text)
        local num = tonumber(text)
        if num then
            stagger = math.max(0, num) -- donÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€¦Ã‚Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€¦Ã‚Â¾ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¢t allow negative
        end
    end,
})
...
BDTab:CreateInput({
    ...
    Callback = function(text)
        local num = tonumber(text)
        if num then
            antiFlickBaseOffset = math.max(0, num) -- donÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€¦Ã‚Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€¦Ã‚Â¾ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¢t allow negative
        end
    end,
})

-- ========== ADDED: AutoBlockAudio Delay SETTINGS FOR BETTER DETECTION ==========
-- Default values (can be changed via BDTab inputs below)
local BD_AUDIO_PERFORM_DELAY = 0.15

BDTab:CreateInput({
    Name = "Delay before BD Audio performs Block (seconds)",
    PlaceholderText = "0.15",
    RemoveTextAfterFocusLost = false,
    Flag = "BDAudioPerformDelay",
    Callback = function(text)
        local num = tonumber(text)
        if num then
            BD_AUDIO_PERFORM_DELAY = math.max(0, num)
        end
    end,
})
...
BDTab:CreateInput({
    Name = "Part Transparency (0 = solid, 1 = invisible)",
    PlaceholderText = tostring(BD_PART_TRANSPARENCY),
    RemoveTextAfterFocusLost = false,
    Callback = function(Text)
        local num = tonumber(Text)
...
-- === Base Part Size (default: 5.5, 7.5, 8.5) ===
local bdBaseX, bdBaseY, bdBaseZ = 5.5, 7.5, 8.5

-- === Behead (126355327951215) ===
local bdBeheadX, bdBeheadY, bdBeheadZ = 5.5, 7.5, 8.5

-- === GashingWound (121086746534252) ===
local bdGashX, bdGashY, bdGashZ = 5.5, 7.5, 8.5

-- Base Size Inputs
BDTab:CreateInput({
    Name = "Base Part Size X",
    PlaceholderText = "5.5",
    RemoveTextAfterFocusLost = false,
    Flag = "BD_BaseX",
    Callback = function(Text)
        local num = tonumber(Text)
        if num then bdBaseX = num end
    end
})
BDTab:CreateInput({
    Name = "Base Part Size Y",
    PlaceholderText = "7.5",
    RemoveTextAfterFocusLost = false,
    Flag = "BD_BaseY",
    Callback = function(Text)
        local num = tonumber(Text)
        if num then bdBaseY = num end
    end
})
BDTab:CreateInput({
    Name = "Base Part Size Z",
    PlaceholderText = "8.5",
    RemoveTextAfterFocusLost = false,
    Flag = "BD_BaseZ",
    Callback = function(Text)
        local num = tonumber(Text)
        if num then bdBaseZ = num end
    end
})

-- Behead Size Inputs
BDTab:CreateParagraph({ Title = "Behead (126355327951215)", Content = "" })
BDTab:CreateInput({
    Name = "Behead Size X",
    PlaceholderText = "5.5",
    RemoveTextAfterFocusLost = false,
...
BDTab:CreateInput({
    ...
    Callback = function(Text)
        local num = tonumber(Text)
        if num then bdBeheadZ = num end
    end
})

-- GashingWound Size Inputs
BDTab:CreateParagraph({ Title = "GashingWound (121086746534252)", Content = "" })
BDTab:CreateInput({
    Name = "GashingWound Size X",
    PlaceholderText = "5.5",
    RemoveTextAfterFocusLost = false,
    Flag = "BD_GashX",
    Callback = function(Text)
        local num = tonumber(Text)
        if num then bdGashX = num end
    end
})
BDTab:CreateInput({
    Name = "GashingWound Size Y",
    PlaceholderText = "7.5",
    RemoveTextAfterFocusLost = false,
    Flag = "BD_GashY",
    Callback = function(Text)
        local num = tonumber(Text)
        if num then bdGashY = num end
    end
})
BDTab:CreateInput({
    Name = "GashingWound Size Z",
    PlaceholderText = "8.5",
    RemoveTextAfterFocusLost = false,
    Flag = "BD_GashZ",
    Callback = function(Text)
        local num = tonumber(Text)
        if num then bdGashZ = num end
    end
})
    ...
    Callback = function()
        pcall(function()
            ...
            if not fakeGui then
                ...
                if not success then
                    warn("ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚ÂÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â¦ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â‚¬Å¾Ã‚Â¢ Failed to load Fake Block GUI:", result)
                end
            else
                fakeGui.Enabled = true
                print("ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â¦ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€¦Ã¢â‚¬Å“ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¦ Fake Block GUI enabled")
            end
        end)
    end
})
...
local function addESP(obj)
    ...
    if not plr then return end -- ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â¦ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€¦Ã¢â‚¬Å“ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¦ only add ESP if it's a player character
...
local function playCustomAnim(animId, isPunch)
    ...
    if success and track then
        print("ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â¦ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€¦Ã¢â‚¬Å“ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¦ Playing custom " .. (isPunch and "punch" or "block") .. " animation:", animId)
        track:Play()
        if isPunch then
            lastPunchTime = now
        else
        ...
        warn("ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚ÂÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â¦ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â‚¬Å¾Ã‚Â¢ Failed to load or play custom animation: " .. animId)
    end
end
...
local function _attemptForSound(sound, idParam, mode)
    ...
    if mode == "Block" then
        if antiFlickOn then
            task.spawn(function()
                task.wait(BD_AUDIO_PERFORM_DELAY or 0)
                fireGuiBlock()
            ...
            local now = tick()
            local chosenDelay = tonumber(autoBlockAudioDelay) or 0

            if now >= nextAudioBlockTime then
                nextAudioBlockTime = now + chosenDelay
                task.spawn(function()
                    if chosenDelay > 0 then
                        local startT = tick()
                        while tick() - startT < chosenDelay do
                            task.wait()
                        end
                    end
                    fireGuiBlock()
...
-- ================================================================
-- ÃƒÆ’Ã‚Â°Ãƒâ€¦Ã‚Â¸Ãƒâ€¦Ã‚Â¸Ãƒâ€šÃ‚Â¦ Fungsi utama Better Detection (attemptBDParts)
-- ================================================================
...
local function attemptBDParts(sound)
    ...
    local t = tick()
    if soundBlockedUntil[sound] and t < soundBlockedUntil[sound] then
        return
    end
    ...
    if antiFlickOn then
        -- Ekstrak ID animasi
        local animId = extractNumericSoundId(sound)

        -- Tentukan ukuran DASAR berdasarkan ID
        local basePartSize
        if animId == "126355327951215" then
            basePartSize = Vector3.new(bdBeheadX, bdBeheadY, bdBeheadZ)
        elseif animId == "121086746534252" then
            basePartSize = Vector3.new(bdGashX, bdGashY, bdGashZ)
        else
            basePartSize = Vector3.new(bdBaseX, bdBaseY, bdBaseZ)
        end

        -- Terapkan multiplier global seperti biasa
        ...
        task.spawn(function()
            ...
            for i = 1, count do
                ...
                part.Size = partSize
                part.Transparency = BD_PART_TRANSPARENCY  -- letakkan di sini
                part.Anchored = true
                ...
                -- Cek apakah karakter kita berada di dalam part prediksi BD
                ...
                -- Saat terdeteksi block, lakukan delay sebelum eksekusi block
                if blocked then
                    if not (facingCheckEnabled and not isFacing(myRoot, hrp)) then

                        -- ÃƒÆ’Ã‚Â°Ãƒâ€¦Ã‚Â¸Ãƒâ€¦Ã‚Â¸Ãƒâ€šÃ‚Â¨ Delay sebelum block dilakukan
                        if BD_AUDIO_PERFORM_DELAY and BD_AUDIO_PERFORM_DELAY > 0 then
                            task.wait(BD_AUDIO_PERFORM_DELAY)
                        end

                        -- Lakukan block sesuai mode
                        if autoblocktype == "Block" then
                            fireGuiBlock()
                            if doubleblocktech and cachedCharges and cachedCharges.Text == "1" then
                                fireGuiPunch()
                        ...
                        -- Batasi block agar tidak terlalu cepat
                        soundBlockedUntil[sound] = t + 1.2
                    end
                    break
                end

                -- Delay antar spawn part
